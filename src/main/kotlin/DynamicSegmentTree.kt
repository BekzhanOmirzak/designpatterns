class DynamicSegmentTree(val size: Int) {
    ...
    fun update(value: Int, max: Int) {
        updateHelper(root, value, 0, 0, size, max)
    }

    private fun updateHelper(root: Pointer, value: Int, at: Int, l: Int, r: Int, max: Int) {
        if (value !in l..r)
            return
        else if (l == r) {
            root.freq = max
        ...
        val mid = (l + r) / 2
        root.init(at)
        updateHelper(root.left!!, value, at * 2 + 1, l, mid, max)
        updateHelper(root.right!!, value, at * 2 + 2, mid + 1, r, max)
    }
    ...
    fun findMaxInRange(l: Int, r: Int): Int {
